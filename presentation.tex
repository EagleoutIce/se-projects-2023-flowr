...
\scalebox{.8}{\begin{tikzpicture}[br/.style={fill=BackGray!80,minimum width=2.5cm,minimum height=1.5cm,signal,signal from=west,signal to=east,signal pointer angle=125},k/.style={below,font=\footnotesize,xshift=-.33mm,color=darkgray},m/.style={above,font=\scriptsize,xshift=-.33mm,color=gray},a/.style={},BaseGray]
   \tikzset{a/.style={opacity=.45}}
      ...
      \endscope
   \draw[decorate,decoration={brace,mirror},line cap=round,line join=round] ([xshift=-1cm,yshift=-6mm]@l) to[edge node={node[below] {\includegraphics[height=2\baselineskip]{data/flowR.pdf}}}] ([xshift=1mm,yshift=-6mm]@r);
\end{tikzpicture}}
\end{lrbox}

\begin{frame}[fragile,c]{}
\only<3->{\vspace*{-1em}}\only<-2|handout:0>{\vspace*{4em}}\begin{layout-full}\begin{center}%
\lstfs{9}
\begin{tikzpicture}
\node (@) at (0,0) {%
...
\AnimateCode{onslide={o8:{7,5,4,3,1},-}, first slide=2,handout={2/1}}
\begin{minted}[deletekeywords={sum,prod},escapeinside={/*}{*/}]{R}
sum  <- 0
prod <- 1
n    <- 10
...
\end{uncoverenv}
\onslide<2->{
   \draw[-Kite,line cap=round] ([xshift=5mm]@.east) to[edge node={node[above] {\scriptsize\T{slice(\solGet{linenumbers}{\footnotesize10}, \textbf{sum})}}}] ([xshift=-5mm]@2.west);
}
\end{tikzpicture}
\begin{tikzpicture}[overlay,remember picture]
